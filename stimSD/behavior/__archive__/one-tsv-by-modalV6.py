#!/usr/bin/env python3
"""
Rassemble tous les TSV BIDS beh (bids-V6-eCRF) en un seul fichier par tâche,
en ajoutant les colonnes participant_id et session extraites du nom de fichier.

Output :
  /Volumes/.../beh/task-<label>_beh_eCRF_V6.tsv   (toutes les tâches V6)
"""
import logging
import re
import pandas as pd
...
BIDS_ROOT = Path("/Volumes/levy/raw/valerocabre/stimSD/Data/derivatives/bids-V6-eCRF/")
...
_log_file = _LOG_DIR / f"one-tsv-by-modalV6_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.DEBUG,
    format="%(message)s",
    handlers=[
...
for task in TASKS:
    ...
    out_file = OUT_DIR / f"task-{task}_beh_eCRF_V6.tsv"
