...
xlsx_ecrf_list = patients_pairs + temoins_pairs

log.info(f"\n🔍 {len(patients_pairs)} fichiers XLSX patients + {len(temoins_pairs)} témoins = {len(xlsx_ecrf_list)} total")
...
log.info("=" * 70)
log.info(f"  XLSX traités    : {len(xlsx_ecrf_list)} ({len(patients_pairs)} patients + {len(temoins_pairs)} témoins)")
