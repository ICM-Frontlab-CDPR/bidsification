...
SHEET_CONFIGS: dict[str, SheetConfig] = {
    ...
    "DENO": _c(
        ...
        _BASE + [
            ...
            ("response",      r"^r[ée]ponse(\s*\.\d+)?$"),  # tolère Réponse.1, Réponse .1
            ("score_manual",  r"^score"),
        ],
        req=["session", "trial"],          # accuracy absente si pas de col Slide1
        opt={"stimulus": "n/a", "status": "n/a", "response_time": "n/a",
             "accuracy": "n/a", "response": "n/a", "score_manual": "n/a"},
    ...
    "LECTURE": _c(
        ...
        _BASE + [
            ("status",     r"^status"),
            ("stimulus",   r"^stimulus"),
            ("accuracy",   r"^r[ée]ponse(\s*\.\d+)?$"),  # tolère Réponse.1, Réponse .1
            ("api_errors", r"^api"),
        ],
        req=["session", "trial", "accuracy"],
...
log.info(f"\n🔍 {len(patients_pairs)} fichiers XLSX patients + {len(toni_pairs)} toni_data + {len(temoins_pairs)} témoins = {len(xlsx_ecrf_list)} total")
for f, ecrf in xlsx_ecrf_list:
    log.info(f"  - [{ecrf}] {f.parent.name}/{f.name}")
log.info("")

resp = input(f"⚠️  Convertir {len(xlsx_ecrf_list)} fichiers en BIDS beh ? (o/n) : ")
...
log.info("=" * 70)
log.info(f"  XLSX traités    : {len(xlsx_ecrf_list)} ({len(patients_pairs)} patients + {len(toni_pairs)} toni_data + {len(temoins_pairs)} témoins)")
