...
def load_resolutions(path: Path | None) -> dict[str, str]:
    """Charge un CSV optionnel 'source,resolved' pour trancher à l'avance les
    ambiguïtés connues (colonnes requises : source, resolved)."""
    if path is None:
        return {}
    if not path.is_file():
        raise SystemExit(f"Fichier de résolution introuvable : {path}")
    df = pd.read_csv(path, dtype=str)
    if not {"source", "resolved"}.issubset(df.columns):
        raise SystemExit(f"{path} doit avoir les colonnes 'source,resolved'.")
    return {
        str(s).strip(): str(r).strip()
        for s, r in zip(df["source"], df["resolved"])
        if str(s).strip()
    }
...
def load_mapping(
    ...
    Si la convention source contient des valeurs dupliquées pointant vers des
    cibles différentes, le comportement dépend de ``on_ambiguous`` :

        "error"       - (défaut) arrête le script avec le détail des conflits.
        "interactive" - demande à l'utilisateur de choisir, cas par cas.
        "first"       - garde la première cible rencontrée dans le xlsx.
        "last"        - garde la dernière cible rencontrée dans le xlsx.

    ``resolutions`` (optionnel) fournit des choix {source: cible} décidés à
    l'avance ; ils sont prioritaires sur ``on_ambiguous`` pour les IDs
    concernés.
    """
    ...
    for src, dst in zip(df[src_conv], df[dst_conv]):
        ...
        if not d:
            ...
            continue
        candidates[s].append(d)

    mapping: dict[str, str] = {}
    ambiguous: dict[str, list[str]] = {}

    for s, dsts in candidates.items():
        uniq = list(dict.fromkeys(dsts))  # dédoublonne en gardant l'ordre d'apparition
        if s in resolutions:
            chosen = resolutions[s]
            if chosen not in uniq:
                raise SystemExit(
                    f"Résolution invalide pour {s!r} : {chosen!r} n'est pas "
                    f"parmi les cibles possibles {uniq} (fichier --resolve)."
                )
            mapping[s] = chosen
        elif len(uniq) == 1:
            mapping[s] = uniq[0]
    ...
    if ambiguous:
        if on_ambiguous == "error":
            details = "\n".join(f"  {k!r} -> {v}" for k, v in ambiguous.items())
            raise SystemExit(
                f"Convention source {src_conv!r} ambiguë (valeurs dupliquées vers "
                f"des cibles différentes) :\n{details}\n"
                f"Utilise --on-ambiguous {{interactive,first,last}}, ou --resolve "
                f"FICHIER.csv (colonnes: source,resolved), ou choisis une "
                f"convention source unique (eCRF, Excel, CENIR, sub_eCRF_BIDS ou "
                f"eCRF_ID)."
            )
        elif on_ambiguous == "first":
            for s, dsts in ambiguous.items():
                mapping[s] = dsts[0]
        elif on_ambiguous == "last":
            for s, dsts in ambiguous.items():
                mapping[s] = dsts[-1]
        elif on_ambiguous == "interactive":
            for s, dsts in ambiguous.items():
                print(f"\nAmbigu pour {src_conv} = {s!r} :", file=sys.stderr)
                for i, d in enumerate(dsts, 1):
                    print(f"  [{i}] {d}", file=sys.stderr)
                while True:
                    choice = input(f"Choix pour {s!r} (1-{len(dsts)}) : ").strip()
                    if choice.isdigit() and 1 <= int(choice) <= len(dsts):
                        mapping[s] = dsts[int(choice) - 1]
                        break
                    print("Entrée invalide.", file=sys.stderr)
        else:
            raise SystemExit(f"Stratégie --on-ambiguous inconnue : {on_ambiguous!r}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--on-ambiguous", choices=("error", "interactive", "first", "last"),
                   default="error",
                   help="Comportement si la convention source a des valeurs dupliquées "
                        "pointant vers des cibles différentes (défaut : error).")
    p.add_argument("--resolve", type=Path, default=None,
                   help="CSV (colonnes: source,resolved) pour trancher à l'avance des "
                        "ambiguïtés connues, sans interaction.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    resolutions = load_resolutions(args.resolve)
    mapping = load_mapping(
        args.xlsx, args.sheet, args.from_conv, args.to_conv,
        on_ambiguous=args.on_ambiguous, resolutions=resolutions,
    )
